...
- **Tabelas Principais:** `accounts`, `characters`, `character_inventory`, `audit_events`, `audit_logs`, `whitelist_applications`, `staff_roles`, `factions`, `holds`, `properties`, `market_stalls`, `crafting_recipes`, `crafting_ingredients`. O schema completo está em `skymp/packages/database/schema.sql` mais as migrations `v2`–`v17`, aplicadas **em ordem** (v6 = `launch_tickets`, v7 = índices das queries quentes, v8 = `game_sessions`, v9 = `characters.gold`, v10 = Afinidade da Alma, v11 = ledger de tesouros institucionais, v12 = ledger do mercado regional, v13 = idempotência de vendas em barracas, v14 = Inventory Framework, v15 = Economy Framework, v16 = silêncio de staff na voz, que sobrevive ao restart, v17 = `audit_events`, a auditoria saindo de dentro do fluxo de evento de jogo).
...
- **Log de moderação** (`moderationLog.js`, endpoint interno `POST /api/moderation-log`): posta um embed num canal configurável (`MODERATION_LOG_CHANNEL_ID`) a cada ação de moderação. Era a intenção original registrada aqui e ficou anos sem implementação; entrou em 07/08/2026.
  ...
  **O canal não é o registro — é notificação.** O registro é `audit_events`, escrito pelo gamemode e pelo painel no mesmo fluxo da ação, antes de qualquer coisa sair para o Discord. (Era `audit_logs` até 15/08/2026; a auditoria mudou de tabela porque aquela recebia treze escritores e afogava em evento de jogo — ver [`admin/AUDIT_LOG.md`](admin/AUDIT_LOG.md).) A distinção decide o comportamento em falha: se o Discord estiver fora, a ação de moderação acontece do mesmo jeito, nada é desfeito e nada fica lento. O endpoint responde **202 antes** de falar com o Discord, e nenhum produtor faz `await` do envio.
  ...
  **`ban` está declarado e não tem produtor.** Não existe `/ban` no gamemode nem no painel. O tipo de evento fica declarado (com teste travando o formato) para que o dia em que o comando existir custe uma linha, mas o log não inventa uma ação que o servidor não tem.

  Até 15/08/2026 a permissão `ban` era **concedida** a `admin` e `owner` e verificada em lugar nenhum — uma porta que descrevia um poder inexistente. Hoje ela é `players.ban`, marcada como **reservada** no catálogo (`core/permissions.js`): o nome fica tomado para ninguém inventar um rival, e ela nega para todo cargo, `owner` incluído, com o motivo `reserved_permission` no log. Há teste que reprova quem conceder uma reservada a um cargo.

  **Por que push e não polling do registro.** O bot tem `mysql2` em `dependencies` sem usar, então ler a tabela era possível. Descartado: daria credencial de banco a um terceiro processo para ler o que ele não escreve, em troca de latência de polling. O push sai de onde a ação acontece, e o único segredo que atravessa é o `INTERNAL_API_SECRET` que o painel já compartilha com o bot. O gamemode usa `http.request` do core em vez de `fetch` — a versão do Node embutida no SkyMP não é controlada por nós, e `fetch` global só existe do Node 18 em diante.
