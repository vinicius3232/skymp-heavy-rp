...
- **Tipagem da API `mp`**: `skymp/gamemode/types/mp.d.ts` (o SkyMP não publica typings). O gamemode continua JS puro carregado direto pelo servidor, sem passo de build. `npm run typecheck` é o gate do código ativo: falha por regressão ativa ou de dependência e mantém a dívida dos seis módulos `PARKED` visível sem misturar os dois estados. `npm run typecheck:all` exige zero diagnóstico inclusive nos estacionados. Política e baseline em [`docs/technical/TYPECHECK_POLICY.md`](technical/TYPECHECK_POLICY.md).
