...
- **SkyVoice — a voz virou subsistema próprio (14/08/2026)**: quem decide quem ouve quem saiu do `voip-service` e virou o **Voice Core**, com índice espacial e política de personagem numa porta só (morto, inconsciente, abatido, amordaçado, silenciado pela staff). Há captura de microfone real por um **helper nativo WASAPI** fora da CEF, um **gateway LiveKit** validado contra um `livekit-server 1.13.5` de verdade, áudio espacial na CEF 108, diagnóstico no painel de staff e auditoria de segurança e privacidade executável. **O veredito é ❌ NÃO É PRODUCTION-READY, e o motivo não é técnico: ninguém nunca ouviu a voz deste projeto** — nenhum cliente Skyrim conectou, e não existe cliente LiveKit dos dois lados. Comece por [SKYVOICE_PRODUCTION_READINESS.md](docs/technical/SKYVOICE_PRODUCTION_READINESS.md).
...
- **Três frameworks são caminho obrigatório (13/08/2026)**: item que muda de dono passa pelo [Inventory Framework](docs/framework/INVENTORY_FRAMEWORK.md) (migration v14); dinheiro que se move passa pelo [Economy Framework](docs/framework/ECONOMY_FRAMEWORK.md) (migration v15, contas e ledger — [ADR 004](docs/technical/ADR_004_ECONOMY_ACCOUNTS_AND_LEDGER.md)); e clique em alvo passa pelo [Interaction Framework](docs/framework/INTERACTION_FRAMEWORK.md), cujo caminho legado foi **removido**. Cada um tem auditoria e matriz de teste próprias; **nenhum rodou numa sessão real**.

- **Identidade online resolvida no servidor**: o `profileId` que chega pelo caminho online resolve `accounts.id` no próprio servidor, em vez de ser aceito como o cliente informa — ver [ADR 001](docs/technical/ADR_001_ONLINE_PROFILE_ID_IS_ACCOUNT_ID.md). Com `offlineMode: true` o cliente continua declarando o próprio `profileId`, e isso é laboratório, não produção.
